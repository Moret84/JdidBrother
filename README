...
1/Un dossier config, dans lequel on trouvera tous ce qui est nécéssaire à la configuration du script, à savoir les fichiers suivants :  
	A/Le fichier config/companiesList.conf, qui est le fichier qui contient les noms des entreprises qu'il est possible de suivre en utilisant le script, auxquelles sont associés leur symbole boursier.  
	B/Le fichier config/help.conf, contenant l'aide du programme.  
	
	Au cours de l'utilisation du script, le dossier config se garnit de fichiers supplémentaires :  
	C/Le fichier config/watchList.conf, qui va contenir la liste des symboles boursiers des entreprises configurées comme à surveiller.  
	D/Le fichier config/settings.conf, contenant tous les réglages inhérents à l'exécution automatique du script.  
...
2/Un dossier data, contenant les données + graphiques, un sous dossier par entreprise à surveiller, chaque sous-dossier contient:  
	A/un fichier prices.dat, qui contient tous les cours de l'entreprise, précédé de la date de ce prix, le plus récent étant en bas.  
	B/Les graphiques représentant les cours en fonction du temps . Ces graphiques possèdent le masque de nom JJMMYYYY_HHMMSS.png .  
